...
import { ReschduleUserAppint } from "../services/url.js";

const router = useRouter();
const route = useRoute();
const ReschedDate = ref(new Date());
const Toaster = ref(false);
const selectedTime = ref("");

const formattedDate = computed(() => {
  if (!ReschedDate.value) return "";
  const d = new Date(ReschedDate.value);
...
const NextStep = async () => {
  try {
    const response = await ReschduleUserAppint.patch(`/reschedule`, {
      UserAppointID: UserAppointID,
      selectedTime: selectedTime.value,
      ReschedDate: formattedDate.value,
    });

    if (!ReschedDate.value || !selectedTime.value) {
      Toaster.value = true;
      setTimeout(() => {
        Toaster.value = false;
      }, 3000);
